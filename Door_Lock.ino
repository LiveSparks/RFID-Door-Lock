...
void setup() {
  ...
  servo_lock.attach(4);               // Configure Servo to Pin 4
  servo_lock.write(90);               // Make sure door is locked
  ...
  Serial.println(F("Relay Door Control"));   // For debugging purposes
  ShowReaderDetails();                       // Show details MFRC522 Card Reader details
...
void loop () {
  do {
    ...
    if (digitalRead(doorUnlock) == LOW){  // Open the door if button is pressed from the inside
  ...
  if (programMode) {
    ...
    if ( isMaster(readCard)) {    // If scanned card's ID matches Master Card's ID - enter program mode
      ...
      if ( findID(readCard) ) { // If not, see if the card is in the EEPROM
        Serial.println(F("Welcome, You shall pass"));
        granted(7000);         // Open the door lock for 5 seconds
      }
...
void granted ( uint16_t setDelay) {
  ...
  servo_lock.write(0);              // Unlock door!
  delay(setDelay);                  // Hold door lock open for given seconds
  servo_lock.write(90);             // Relock door
